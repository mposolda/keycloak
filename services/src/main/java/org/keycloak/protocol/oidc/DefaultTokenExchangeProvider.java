...
public class DefaultTokenExchangeProvider implements TokenExchangeProvider {
    ...
    protected Response exchangeClientToClient(UserModel targetUser, UserSessionModel targetUserSession,
        ...
        // TODO:mposolda check if client_id details is already in the event at this stage
        ...
        boolean isClientTheAudience = targetAudienceClients.isEmpty();
        ...
        // Comparing requested "scope" with the available client scopes of the client, which is doing the token exchange
        String scope = formParams.getFirst(OAuth2Constants.SCOPE);
        if (token != null && token.getScope() != null && scope == null) {
            scope = token.getScope();

            Set<String> targetClientScopes = new HashSet<String>();
            targetClientScopes.addAll(client.getClientScopes(true).keySet());
            targetClientScopes.addAll(client.getClientScopes(false).keySet());
        ...
        try {
            ...
            switch (requestedTokenType) {
                ...
                case OAuth2Constants.SAML2_TOKEN_TYPE:
                    return exchangeClientToSAML2Client(targetUser, targetUserSession, requestedTokenType, targetAudienceClients.get(0)); // TODO:mposolda should update SAML as well?
            }
        } finally {
            session.getContext().setClient(client);
        }
...
//        }
        ...
        if (params.getAudience() != null) {
            // TODO:mposolda need to doublecheck what to do with existing audiences. Clearing for now...
            responseBuilder.getAccessToken().audience();
