...
public class PasskeysUsernamePasswordFormTest extends AbstractWebAuthnVirtualTest {
    ...
    // Test user is able to authenticate with password during re-authentication when the passkeys is not configured for the user (for example when OIDC parameter prompt=login is used)
    @Test
...
//                    .assertEvent();
    ...
    // TODO:mposolda test for re-authentication of the user with passkey. Testing authentication with password for this user. First check incorrect password authentication of this user and check that webauthn elements still available
}
